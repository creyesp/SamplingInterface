...
if ~matlab.internal.strfun.ischarlike(str)
...
if nargin > 2
    ...
    for vargIdx = 1:numel(varargin)
        if isscalar(varargin{vargIdx}) && isstring(varargin{vargIdx})
            varargin{vargIdx} = char(varargin{vargIdx});
        end
    end
    p.parse(varargin{:});
